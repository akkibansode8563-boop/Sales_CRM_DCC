...
export async function authLogin(username, password) {
  ...
  // Auto-fix usernames with spaces or capitals
  let changed = false
  db.users.forEach(u => {
  ...
  // ✅ Capture plain_password on every successful login so Admin can always see it
  const idx = db.users.findIndex(u => u.id === user.id)
  if (idx !== -1 && db.users[idx].plain_password !== password.trim()) {
    db.users[idx].plain_password = password.trim()
    saveDB(db)
  }
...
// Admin-only: returns users including plain_password for display
