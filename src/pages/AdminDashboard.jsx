...
import { getCustomers, getAllVisits as getAllVisitsAll } from '../utils/supabaseDB'
...
export default function AdminDashboard() {
  ...
  const reload = useCallback(() => {
    ...
    setAllVisitsData(getAllVisitsAll())
  ...
  const loadAnalytics = useCallback((period, date, mgrId=null) => {
    setAnalyticsData(getAnalytics(mgrId, period, date))
  }, [])
  ...
  const totalVisitsToday = managers.reduce((s,m)=>s + (m.visits_today || 0), 0)
  const activeJourneys   = managers.filter(m=>m.active_journey).length
  const totalSalesToday = managers.reduce((s,m)=>s + (m.today_sales || 0), 0)
