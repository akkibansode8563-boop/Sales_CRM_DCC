...
export default memo(function CustomerDatabase({ customers = [], visits = [], managers = [] }) {
  ...
  const [mgrFilter, setMgrFilter] = useState('all')   // NEW: manager filter
  ...
  const enriched = useMemo(() => customers.map(c => {
    ...
    // Which managers have visited this customer?
    const visitingMgrIds = [...new Set(cVisits.map(v => v.manager_id).filter(Boolean))]
    const visitingMgrs   = visitingMgrIds.map(id => managers.find(m => m.id === id)).filter(Boolean)
    return { ...c, cVisits, lastVisit, totalSales, createdBy, days, priority, visitingMgrs, visitingMgrIds }
  ...
  const filtered = useMemo(() => {
    ...
    // Filter by manager: show customers that were CREATED by OR visited by the selected manager
    if (mgrFilter !== 'all') {
      const mid = Number(mgrFilter)
      list = list.filter(c => c.created_by === mid || c.visitingMgrIds?.includes(mid))
    }
  ...
  const exportExcel = useCallback(() => {
    ...
    const rows = toExport.map((c, i) => ({
      ...
      'Visited By':     c.visitingMgrs?.map(m => m.full_name).join(', ') || '',
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', gap:8, flexWrap:'wrap', alignItems:'center', marginBottom:12 }}>
        ...
        {/* Manager filter — NEW */}
        <select value={mgrFilter} onChange={e => { setMgrFilter(e.target.value); setSelIds(new Set()) }}
          style={{ padding:'7px 10px', border:'1.5px solid #E5E7EB', borderRadius:8, fontSize:'0.78rem', color:'#374151', background:'#F9FAFB', cursor:'pointer', maxWidth:170 }}>
        ...
        <span style={{ display:'flex', alignItems:'center', gap:6, fontSize:'0.7rem', color:'#9CA3AF', fontWeight:600 }}>
          {filtered.length} of {enriched.length} customers
          {mgrFilter !== 'all' && (
            <button onClick={() => setMgrFilter('all')} style={{
              background:'#EFF6FF', border:'none', borderRadius:99, padding:'2px 8px',
              color:'#2563EB', fontWeight:700, fontSize:'0.65rem', cursor:'pointer', fontFamily:'inherit',
              display:'flex', alignItems:'center', gap:3,
            }}>
              ✕ {managers.find(m=>String(m.id)===mgrFilter)?.full_name?.split(' ')[0] || 'Manager'}
            </button>
          )}
        </span>
      </div>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10, padding:'11px 14px', cursor:'pointer' }}
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:2, flexWrap:'wrap' }}>
                      ...
                      {c.visitingMgrs?.length > 0 && (
                        <span style={{ fontSize:'0.6rem', color:'#7C3AED', background:'#F5F3FF', padding:'1px 6px', borderRadius:99 }}>
                          👤 {c.visitingMgrs.map(m => m.full_name.split(' ')[0]).join(', ')}
                        </span>
                      )}
                    </div>
                  </div>
