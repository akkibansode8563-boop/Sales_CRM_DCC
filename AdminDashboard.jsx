...
import {
  getUsers, getUsersAdmin, createUser, updateUser, deleteUser, adminSetPassword, removeDuplicateUsers,
...
export default function AdminDashboard() {
  ...
  const doCreateUser = async () => {
    const cleanUser = uf.username.trim().toLowerCase().replace(/\s+/g,'_')
    if (!cleanUser||!uf.full_name.trim()) return toastMsg('Username & full name required','error')
    if (!editingUser&&!uf.password.trim()) return toastMsg('Password is required','error')
    if (!editingUser&&uf.password.trim().length<4) return toastMsg('Password min 4 characters','error')
    const finalUf = {...uf, username:cleanUser, full_name:uf.full_name.trim()}
    try {
      if (editingUser) { await updateUser(editingUser.id,finalUf); toastMsg('User updated ✅') }
      else { await createUser(finalUf); toastMsg('User "'+cleanUser+'" created ✅ pwd: '+uf.password.trim()) }
  ...
  const doAdminSetPwd = async (userId, newPwd) => {
    if (!newPwd || newPwd.trim().length < 4) return toastMsg('Min 4 characters required', 'error')
    try {
      await adminSetPassword(userId, newPwd.trim())
      setPwdEdit({})
      reload()
  ...
  const doRemoveDuplicates = () => {
    const count = removeDuplicateUsers()
    reload()
    toastMsg('Duplicates removed. ' + count + ' users remain ✅')
  }
            ...
            <div className="panel">
              <div className="panel-hdr">
                ...
                <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
                  ...
                  <button className="panel-action" onClick={doRemoveDuplicates} style={{color:'#ef4444'}}>🧹 Fix Duplicates</button>
                ...
                {users.map((u,i)=>(
                  <div key={u.id} className="user-row">
                    ...
                    <div className="ur-info" style={{flex:1,minWidth:0}}>
                      ...
                      {/* Always show password row for Admin */}
                      {user?.role==='Admin' && (
                        <div style={{display:'flex',alignItems:'center',gap:'6px',marginTop:'4px',flexWrap:'wrap'}}>
                          ...
                          {u.plain_password ? (
                            ...
                            <span style={{fontSize:'0.72rem',color:'#f59e0b',fontWeight:600}}>
                              ...
                              <button onClick={()=>setPwdEdit(p=>({...p,[u.id]:''}))}
                                style={{color:'#2563eb',background:'none',border:'none',cursor:'pointer',textDecoration:'underline',padding:0,fontSize:'0.72rem'}}>
                                set now
                              </button>
                            </span>
                          )}
                          {pwdEdit[u.id] !== undefined && (
                            <div style={{display:'flex',gap:'4px',alignItems:'center',width:'100%',marginTop:'4px'}}>
                              <input
                                ...
                                style={{flex:1,padding:'4px 8px',border:'1px solid #93c5fd',borderRadius:'6px',fontSize:'0.8rem',minWidth:0}}
                                autoFocus
                              />
                              <button onClick={()=>doAdminSetPwd(u.id,pwdEdit[u.id])}
                                style={{padding:'4px 10px',background:'#2563eb',color:'#fff',border:'none',borderRadius:'6px',cursor:'pointer',fontSize:'0.78rem',fontWeight:700,flexShrink:0}}>
                                Save
                              </button>
                              <button onClick={()=>setPwdEdit(p=>{const n={...p};delete n[u.id];return n})}
                                style={{padding:'4px 8px',background:'#f3f4f6',color:'#374151',border:'none',borderRadius:'6px',cursor:'pointer',fontSize:'0.78rem',flexShrink:0}}>
                                ✕
                              </button>
                            </div>
                          )}
                    ...
                    <div className="ur-actions">
                      ...
                      {user?.role==='Admin' && u.role!=='Admin' && <button className="ur-btn ur-btn-del" onClick={()=>doDeleteUser(u.id,u.full_name)} title="Delete">🗑️</button>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="modal-body">
                  ...
                  <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'8px'}}>
                    <div>
                      ...
                      <div style={{background:'#fff',border:'1px solid #93c5fd',borderRadius:'7px',padding:'7px 10px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'6px'}}>
                        <code style={{fontSize:'0.88rem',color:'#1e3a8a',fontWeight:800,flex:1,wordBreak:'break-all'}}>{editCreds.username}</code>
                        <button type="button" onClick={()=>{navigator.clipboard.writeText(editCreds.username);toastMsg('Username copied!')}}
                          style={{background:'none',border:'none',cursor:'pointer',fontSize:'1rem',flexShrink:0}}>📋</button>
                      </div>
                    </div>
                    <div>
                      <div style={{fontSize:'0.65rem',fontWeight:700,color:'#374151',marginBottom:'4px'}}>
                        PASSWORD {!editCreds.password&&<span style={{color:'#f59e0b'}}> ⚠ not captured yet</span>}
                      </div>
                      <div style={{background:'#fff',border:'1px solid #93c5fd',borderRadius:'7px',padding:'7px 10px',display:'flex',alignItems:'center',justifyContent:'space-between',gap:'4px'}}>
                        <code style={{fontSize:'0.88rem',color:editCreds.password?'#1e3a8a':'#9ca3af',fontWeight:800,flex:1}}>
                          {editCreds.password?(editCreds.show?editCreds.password:'••••••••'):'ask user to login once'}
                        </code>
                        {editCreds.password&&<>
                          <button type="button" onClick={()=>setEditCreds(p=>({...p,show:!p.show}))}
                            style={{background:'none',border:'none',cursor:'pointer',fontSize:'1rem',flexShrink:0}}>{editCreds.show?'🙈':'👁️'}</button>
              ...
              <div className="row-2">
                <div className="fg">
                  ...
                  <select value={uf.territory} onChange={e=>setUf(p=>({...p,territory:e.target.value}))}>
                    <option value="">— Select Territory —</option>
                    {['Mumbai West','Mumbai East','Mumbai Central','Pune City','Pune Suburbs','Nashik','Aurangabad','Thane','Navi Mumbai','Raigad'].map(t=><option key={t}>{t}</option>)}
                  </select>
                </div>
                <div className="fg">
                  <label>Role</label>
                  <select value={uf.role} onChange={e=>setUf(p=>({...p,role:e.target.value}))}><option>Sales Manager</option><option>Admin</option></select>
                </div>
              </div>

              {/* ─── NEW USER CREDENTIALS PREVIEW ─── */}
