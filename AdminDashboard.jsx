...
import {
  ...
  getAllVisitsSync, getDailyReportsSync, getProductEntriesSync,
  getTargetsSync, getJourneyHistorySync, getLiveStatusSync,
  getUsersAdminSync, getAllVisitsAllSync, getCustomersSync
...
// getCustomers and getAllVisitsAll now imported via sync versions above
...
export default function AdminDashboard() {
  ...
  const reload = useCallback(() => {
    try { setManagers(getLiveStatusSync() || []) }       catch(e) { console.error('getLiveStatus',e); setManagers([]) }
    try { setUsers(getUsersAdminSync() || []) }           catch(e) { console.error('getUsersAdmin',e); setUsers([]) }
    try { setTerrStats(getTerritoryStats() || []) }       catch(e) { console.error('getTerritoryStats',e); setTerrStats([]) }
    try { setAllCustomers(getCustomersSync() || []) }     catch(e) { console.error('getCustomers',e); setAllCustomers([]) }
    try { setAllVisitsData(getAllVisitsAllSync() || []) }  catch(e) { console.error('getAllVisitsAll',e); setAllVisitsData([]) }
  ...
  const buildManagerData = useCallback((date=today) => {
    return salesManagers.map((m,i) => {
      const mVisits = getAllVisitsSync(m.id) || []
      const dayVisits   = mVisits.filter(v=>v.visit_date===date)
      const reports = getDailyReportsSync(m.id) || []
      const dayReport   = reports.find(r=>r.date===date)
      const allProds = getProductEntriesSync(m.id) || []
      const dayProducts = allProds.filter(p=>p.date===date)
      const targets = getTargetsSync(m.id) || []
      ...
      const journeys = getJourneyHistorySync(m.id) || []
