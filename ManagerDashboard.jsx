...
import {
  getCurrentStatus, updateStatus, getTodayVisits, getAllVisits, createVisit,
  getActiveJourney, startJourney, endJourney, getTargets,
  getDailySalesReports, saveDailySalesReport,
  getProductDayEntries, createProductDayEntry, updateProductDayEntry, deleteProductDayEntry,
  calcDistanceKm, calcTravelTime,
  searchCustomers, getRecentCustomers, createCustomer, getCustomers, getTerritories,
...
export default function ManagerDashboard() {
  ...
  /* ── GPS live state (updated by BackgroundGPS subscriber) ── */
  const [gpsCoords,   setGpsCoords]   = useState(null)   // { lat, lng }
  const [gpsError,    setGpsError]    = useState(null)
  const [idleStatus,  setIdleStatus]  = useState({ idle: false, minutes: 0 })
  const [suspFlag,    setSuspFlag]    = useState(null)

  /* ── Live journey timer (ticks every second) ── */
  ...
  /* ── Toast helper ── */
  const toastMsg = useCallback((msg, type='success') => {
  ...
  const reload = useCallback(() => {
    ...
    const j = getActiveJourney(user.id)
    setJourney(j)
  ...
  /* ── Mount: load data + wire GPS ── */
  useEffect(() => {
    const j = reload()

    // If there's already an active journey, resume BackgroundGPS immediately
    if (j?.id && !BackgroundGPS.isActive()) {
      BackgroundGPS.start(j.id, j.manager_id)
    }

    // Subscribe to GPS events
    const unsub = BackgroundGPS.subscribe(evt => {
      if (evt.type === 'LOCATION_UPDATE') {
        setGpsCoords({ lat: evt.latitude, lng: evt.longitude })
        setGpsError(null)
        setIdleStatus(evt.idle || { idle: false, minutes: 0 })
        setSuspFlag(evt.suspicious ? evt.suspicious_reason : null)
      } else if (evt.type === 'GPS_ERROR') {
        setGpsError(evt.message)
      } else if (evt.type === 'GPS_RECOVERED') {
        setGpsError(null)
        toastMsg('GPS signal restored ✅')
      }
    })
  ...
  /* ── Live elapsed timer — counts every second while journey active ── */
  useEffect(() => {
    if (elapsedRef.current) { clearInterval(elapsedRef.current); elapsedRef.current = null }
    if (journey?.start_time) {
      const tick = () => setElapsed(liveElapsed(journey.start_time))
      tick()
  ...
  useEffect(() => {
    const goOnline = () => {
      ...
      if (q.length > 0) {
        ...
        if (ok > 0) { toastMsg(`✅ Synced ${ok} offline action(s)`); reload() }
      }
    }
  ...
  const checkNearby = async () => {
    ...
    if (c) {
      ...
      if (nearby.length > 0) toastMsg(`📍 ${nearby.length} customer(s) nearby!`, 'info')
      else toastMsg('No customers within 200m', 'info')
    } else {
      toastMsg('Could not detect location', 'error')
    }
  }

  /* ── Status ── */
  const changeStatus = s => {
    updateStatus(user.id, s); setStatus(s); setShowStatusPicker(false); toastMsg(`Status → ${s}`)
  }
  ...
  const handleStartJourney = async () => {
    ...
    try {
      ...
      setJourney(j)
      changeStatus('On Field')
      // Immediately start background GPS tracking
      BackgroundGPS.start(j.id, user.id)
      toastMsg('Journey started! 🚀 GPS tracking active')
      setShowMap(true)
  ...
  const handleEndJourney = async () => {
    toastMsg('Ending journey…', 'info')
    const c = await getGPS()
    const loc = c ? await reverseGeo(c.latitude, c.longitude) : 'End Point'
    try {
      // Stop background GPS FIRST
      BackgroundGPS.stop()
      const j = endJourney(user.id, loc, c?.latitude, c?.longitude)
      setJourney(null)
      setGpsCoords(null)
      setIdleStatus({ idle: false, minutes: 0 })
      setSuspFlag(null)
      changeStatus('In-Office')
      reload()
      toastMsg(`Journey complete · ${j.total_visits} stops · ${j.total_km} km 🎯`)
    } catch(e) { toastMsg(e.message, 'error') }
  }
  ...
  const submitVisit = async () => {
    if (!vf.customer_name.trim() || !vf.location.trim())
      return toastMsg('Customer name & location required', 'error')
    // Use BackgroundGPS last known coords, fallback to fresh GPS
    const coords = BackgroundGPS.lastCoords()
    const c = coords ? { latitude: coords.lat, longitude: coords.lng } : await getGPS()
    createVisit({
    ...
    setVisitModal(false); setVf(initVF()); reload(); toastMsg('Visit logged ✅')
  }
  ...
  const submitSales = () => {
    ...
    saveDailySalesReport({
      manager_id: user.id, date: today,
      sales_target: +sf.sales_target || 0, sales_achievement: +sf.sales_achievement || 0,
      profit_target: +sf.profit_target || 0, profit_achievement: +sf.profit_achievement || 0,
    })
    setSalesModal(false); setSf(initSF()); reload(); toastMsg('Report submitted ✅')
  }
  const salesAchPct = sf.sales_target > 0 && sf.sales_achievement > 0
    ? Math.round((+sf.sales_achievement / +sf.sales_target) * 100) : null

  /* ── Product submit ── */
  const submitProduct = () => {
    if (!pf.brand.trim() || !pf.product_name.trim())
      return toastMsg('Brand & product required', 'error')
    if (editProd) {
      ...
      createProductDayEntry({
        manager_id: user.id, date: today,
        brand: pf.brand, brand_id: pf.brand_id || null,
        product_name: pf.product_name, product_id: pf.product_id || null,
        target_qty: +pf.target_qty || 0, achieved_qty: +pf.achieved_qty || 0,
        target_amount: +pf.target_amount || 0, achieved_amount: +pf.achieved_amount || 0,
      })
    }
  ...
  const openEditProd = p => {
    setEditProd(p)
    setPf({ brand: p.brand||'', brand_id: p.brand_id||null, product_name: p.product_name,
            product_id: p.product_id||null, target_qty: String(p.target_qty),
            achieved_qty: String(p.achieved_qty), target_amount: String(p.target_amount),
            achieved_amount: String(p.achieved_amount) })
    setProductModal(true)
  }
  const prodPct = pf.target_qty > 0 && pf.achieved_qty > 0
    ? Math.round((+pf.achieved_qty / +pf.target_qty) * 100) : null
  ...
  const visitPct = latestTarget?.visit_target
    ? Math.min((todayVisits.length / latestTarget.visit_target) * 100, 100) : 0
  const salesPct = latestTarget?.sales_target && todayReport
    ? Math.min((todayReport.sales_achievement / latestTarget.sales_target) * 100, 100) : 0
  ...
  return (
    <div className="mgr-app">
      ...
      <header className="mgr-header">
        <div className="mgr-hdr-top">
          <div className="mgr-user">
            <div className="mgr-avatar">{user?.full_name?.[0]}</div>
            <div>
          ...
          <div className="mgr-hdr-actions">
            {/* GPS live indicator */}
            {BackgroundGPS.isActive() && (
              <div className="hdr-gps-dot" title="GPS tracking active">
                <span className="hdr-gps-pulse"/>
                <span className="hdr-gps-lbl">GPS</span>
              </div>
            )}
        ...
        <div className="mgr-status-row">
          ...
          {journey && (
            <button className="journey-live-pill" onClick={() => setShowMap(true)}>
              ...
              <span className="jlp-txt">{todayVisits.length} stops · {journeyKm.toFixed(1)} km · {elapsed}</span>
            </button>
          )}
        </div>
      </header>
      ...
      {/* ── Idle Alert ── */}
      {idleStatus.idle && journey && (
        <div className="idle-banner">
          ⏸ Idle detected — no movement for {idleStatus.minutes} min
        </div>
      )}

      {/* ── Suspicious GPS Alert ── */}
      {suspFlag && (
      ...
      <main className="mgr-main">
        ...
        {tab === 'home' && (
          <div className="tab-pane">
            ...
            <div className="journey-hero">
              <div className="jh-body">
                ...
                <div className="jh-sub">
                  {journey
                    ? `Started ${fmtTime(journey.start_time)} · ${journey.start_location?.split(',')[0]}`
                    : 'Start your journey to activate GPS tracking, route mapping and visit logging.'}
                </div>
                {journey && (
                  <div className="jh-metrics">
                    ...
                    <div className="jh-metric"><span className="jh-metric-val jh-elapsed">{elapsed}</span><span className="jh-metric-lbl">Duration</span></div>
                    {gpsCoords && <div className="jh-metric"><span className="jh-metric-val" style={{fontSize:'0.68rem',fontFamily:'monospace'}}>{gpsCoords.lat.toFixed(4)}</span><span className="jh-metric-lbl">Lat</span></div>}
                  </div>
                )}
            ...
            {/* GPS Status strip (when journey active) */}
            {journey && (
              <div className={`gps-status-strip ${gpsError ? 'gps-strip-error' : 'gps-strip-ok'}`}>
                {gpsError
                  ? <><span className="gps-dot gps-dot-error"/>GPS: {gpsError}</>
                  : <><span className="gps-dot gps-dot-ok"/>GPS Active · updates every 15s{gpsCoords ? ` · ${gpsCoords.lat.toFixed(5)}, ${gpsCoords.lng.toFixed(5)}` : ''}</>
                }
              </div>
            )}
            ...
            <div className="kpi-grid">
              ...
              <div className="kpi-card">
                <div className="kpi-top">
                  <div className="kpi-ico" style={{background: var_pl}}>💰</div>
              ...
              {journey && (
                <>
                  <div className="kpi-card">
                    <div className="kpi-top"><div className="kpi-ico" style={{background:'#F5F3FF'}}>🛣️</div><span className="kpi-badge" style={{background:'#F5F3FF',color:'#7C3AED'}}>km</span></div>
                  ...
                  <div className="kpi-card">
                    <div className="kpi-top"><div className="kpi-ico" style={{background:'#FFFBEB'}}>⏱️</div><span className="kpi-badge" style={{background:'#FFFBEB',color:'#D97706'}}>live</span></div>
                    <div className="kpi-val kh-elapsed" style={{fontSize:'1.1rem'}}>{elapsed}</div>
              ...
              <div className="ai-card">
                ...
                {suggestions.map((s, i) => (
                  <div key={i} className="ai-item">
                    ...
                    <span className={`ai-priority ${s.priority==='high'?'ai-priority-high':'ai-priority-med'}`}>
                      {s.priority === 'high' ? 'Urgent' : 'Follow-up'}
                    </span>
                  </div>
                ))}
              </div>
        ...
        {tab === 'customers' && (
          <div className="tab-pane">
            ...
            {customers.length === 0
              ...
              : customers
                  .filter(c => !customerFilter || c.name.toLowerCase().includes(customerFilter.toLowerCase()) || c.owner_name?.toLowerCase().includes(customerFilter.toLowerCase()))
                  .map(c => (
        ...
        {tab === 'reports' && (
          <div className="tab-pane">
            ...
            {reports.length === 0
              ? <div className="empty"><div className="empty-ico">📊</div><div className="empty-txt">No reports yet.</div><button className="empty-cta" onClick={() => setSalesModal(true)}>Submit Today's Report</button></div>
              : reports.map(r => {
                ...
                return (
                  <div key={r.id} className="report-card">
                    ...
                    <div className="rc-grid">
                      <div><div className="rc-cell-lbl">Sales</div><div className="rc-cell-val">{fmt(r.sales_achievement)}</div><div className="rc-cell-tgt">of {fmt(r.sales_target)}</div></div>
                      <div><div className="rc-cell-lbl">Profit</div><div className="rc-cell-val">{fmt(r.profit_achievement)}</div><div className="rc-cell-tgt">of {fmt(r.profit_target)}</div></div>
                    </div>
                  ...
                  return (
                    <div key={p.id} className="product-card">
                      ...
                      <div className="pc-metrics">
                        <div className="pc-metric"><div className="pc-metric-lbl">Qty</div><div className="pc-metric-val">{p.achieved_qty}<span style={{color:'#9CA3AF',fontWeight:400}}>/{p.target_qty}</span></div></div>
        ...
        {tab === 'profile' && (
          <div className="tab-pane">
            ...
            <div className="prof-stats">
              {[{v:allVisits.length,l:'Visits'},{v:reports.length,l:'Reports'},{v:products.length,l:'Products'},{v:customers.length,l:'Customers'}].map(s=>(
            ...
            <div className="info-table">
              {[
                ...
                ['Role','Sales Manager'],
                ['Status',`${sm.icon} ${status}`],
                ['Journey',journey?`Active · ${elapsed}`:'Not active'],
                ['GPS Tracking',BackgroundGPS.isActive()?'🟢 Active':'⚪ Off'],
                ...
                ['App Version','v5.0 · DCC SalesForce · PWA'],
              ].map(([k,v]) => (
      ...
      <nav className="tab-bar">
        {[
          ...
          <button key={t.id} className={`tab-btn ${tab===t.id?'tab-active':''}`} onClick={() => setTab(t.id)}>
            <div className="tab-ico-bg"><span className="tab-ico">{t.ico}</span></div>
      ...
      {showMap && (
        <JourneyMap
          ...
          onClose={() => { setShowMap(false); reload() }}
        />
      )}
            ...
            <div className="modal-body">
              <div className="fg">
                ...
                <AutocompleteInput
                  ...
                  searchFn={searchCustomers} recentsFn={getRecentCustomers}
                  renderItem={c => <span className="ac-item-name">{c.name}</span>}
                  renderMeta={c => <span className="ac-type-tag">{c.type}</span>}
                  addNewLabel="+ Add New Customer" onAddNew={() => setAddCustomerModal(true)}
                  autoFocus
                />
              </div>
              <div className="row-2">
                <div className="fg"><label>Client Type</label><select value={vf.client_type} onChange={e => setVf(p=>({...p,client_type:e.target.value}))}>{CLIENT_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>
                <div className="fg"><label>Status</label><select value={vf.status} onChange={e => setVf(p=>({...p,status:e.target.value}))}>{VISIT_STATUSES.map(s=><option key={s}>{s}</option>)}</select></div>
              </div>
              <div className="fg"><label>Visit Type</label><select value={vf.visit_type} onChange={e => setVf(p=>({...p,visit_type:e.target.value}))}>{VISIT_TYPES.map(t=><option key={t}>{t}</option>)}</select></div>
              <div className="fg"><label>Location / Address *</label><input value={vf.location} onChange={e => setVf(p=>({...p,location:e.target.value}))} placeholder="Area, City or full address"/></div>
              <div className="fg"><label>Notes / Outcome</label><textarea value={vf.notes} onChange={e => setVf(p=>({...p,notes:e.target.value}))} placeholder="Orders placed, discussions, follow-ups…" rows={3}/></div>
            </div>
            <div className="modal-foot">
              <button className="btn-cancel" onClick={() => setVisitModal(false)}>Cancel</button>
            ...
            <div className="modal-body">
              ...
              {salesAchPct !== null && (
                <div className="modal-info-box">
                  ...
                  <div><div className="mib-label">Achievement</div><div className="mib-value"><span className="pct-preview">{salesAchPct}%</span> of target</div></div>
                </div>
              )}
      ...
      {productModal && (
        <div className="modal-overlay" onClick={() => {setProductModal(false);setEditProd(null)}}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <div className="modal-hdr"><div className="modal-title">📦 {editProd?'Update':'Add'} Product Entry</div><button className="modal-close" onClick={() => {setProductModal(false);setEditProd(null)}}>✕</button></div>
            <div className="modal-body">
              <div className="fg">
                <label>Brand *</label>
                <AutocompleteInput value={pf.brand}
                  ...
                  placeholder="Search brand…" searchFn={searchBrands} recentsFn={getRecentBrands}
                  renderItem={b => <span className="ac-item-name">{b.name}</span>}
                  addNewLabel="+ Add Brand" onAddNew={() => setAddBrandModal(true)}
                  disabled={!!editProd} autoFocus={!editProd}
                />
              </div>
              <div className="fg">
                <label>Product Name *</label>
                <AutocompleteInput value={pf.product_name}
                  ...
                  placeholder="Search product…" searchFn={q=>searchProducts(q,pf.brand_id||null)} recentsFn={getRecentProducts}
                  ...
                  addNewLabel="+ Add Product" onAddNew={() => setAddProductModal(true)}
                  disabled={!!editProd}
                />
              </div>
              ...
              {prodPct !== null && (
                <div className="modal-info-box">
                  ...
                  <div><div className="mib-label">Qty Achievement</div><div className="mib-value" style={{color:'#7C3AED'}}><span className="pct-preview">{prodPct}%</span> of target</div></div>
                </div>
              )}
            ...
            <div className="modal-foot">
              ...
              <button className="btn-submit" onClick={submitProduct}>{editProd?'Update':'Add Entry'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {addCustomerModal && (
        <QuickAddCustomerModal
          onCreated={c => { setVf(p=>({...p,customer_name:c.name,customer_id:c.id,client_type:c.type,location:c.address||p.location})); toastMsg(`"${c.name}" added ✅`) }}
      ...
      {addProductModal && (
        <QuickAddProductModal
          ...
          onCreated={prod => { setPf(p=>({...p,product_name:prod.name,product_id:prod.id})); toastMsg(`"${prod.name}" added ✅`) }}
          onClose={() => setAddProductModal(false)}
        />
      )}

      {/* ════ ADD CUSTOMER WITH GPS ════ */}
      {showAddCustomer && (
        <AddCustomerModal
          createdBy={user?.id}
          onCreated={c => { reload(); toastMsg(`✅ "${c.name}" added with GPS`) }}
          onClose={() => setShowAddCustomer(false)}
        />
      )}

      {/* ── Banner CSS for idle/susp ── */}
      <style>{`
        .hdr-gps-dot{display:flex;align-items:center;gap:5px;background:#ECFDF5;border:1px solid #A7F3D0;border-radius:99px;padding:4px 9px}
        .hdr-gps-pulse{width:7px;height:7px;background:#10B981;border-radius:50%;animation:ping 1.5s infinite}
        .hdr-gps-lbl{font-size:0.62rem;font-weight:800;color:#059669;letter-spacing:.05em}
        .gps-status-strip{padding:6px 16px;font-size:0.68rem;font-weight:600;display:flex;align-items:center;gap:7px}
        .gps-strip-ok{background:#ECFDF5;color:#059669}.gps-strip-error{background:#FEF2F2;color:#DC2626}
        .gps-dot{width:7px;height:7px;border-radius:50%;flex-shrink:0}
        .gps-dot-ok{background:#10B981;animation:ping 2s infinite}.gps-dot-error{background:#EF4444}
        .idle-banner{background:#FFFBEB;border-bottom:1px solid #FCD34D;padding:7px 16px;font-size:0.72rem;font-weight:700;color:#92400E}
        .susp-banner{background:#FEF2F2;border-bottom:1px solid #FECACA;padding:7px 16px;font-size:0.72rem;font-weight:700;color:#DC2626}
        .jh-elapsed{font-variant-numeric:tabular-nums;letter-spacing:-.02em}
